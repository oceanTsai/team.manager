...
- 所有程式碼已提交並 push，工作區乾淨（最新 commit `d8b3927`）
...
### B. 算出的 Sprint 可能整個在過去 🔴

**位置**：`SprintPlanner.js` 的 `planNext()`

`planNext()` 純粹從「上一個結束日 + 3 天」往後推，**沒檢查結果是否在未來**。

**失效情境**：`prepareRetro` 因故停擺 6 週後才恢復 → 算出一個 6 週前開始、4 週前結束的 Sprint → `schedulePublish()` 拿過去的時間去排排程。

> Apps Script 對過去時間的 `.at()` 是「接受並儘快執行」而非拒絕——此為文件推論，**未實測**。

**要先決定行為**：

| 選項 | 做法 |
|---|---|
| **甲（建議）** | 偵測到過期就拋錯，訊息說明「算出來的是過期 Sprint，請用 `MANUAL_SPRINT` 指定要建哪一個」 |
| 乙 | 連續往後推算，直到算出結束日在未來的那一個（自動追上進度） |

建議甲的理由：符合本專案已定案的原則「不自動修復，失敗就通知讓人決定」（見下方〈已定案的設計決策〉）。乙 等於程式替你決定要跳過幾個 Sprint。
...
1. **B**（真 bug，會產生髒資料）— 需要先決定甲/乙
2. **A**（同一個檔案，順手做掉）
3. **G**（影響最廣，通知靜默失敗）
4. **J → K → L**（部署設定，做完才能真的上線）
5. **E / F**（測試缺口）
6. **C / D / H / I / M**（優先度較低）
7. **五**（未審查的專案，建議一個一個過）
