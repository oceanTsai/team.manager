/**
 * ============================================================
 * SprintService.gs - Sprint 回顧自動化業務邏輯
 * ============================================================
 * Sprint 業務規則 + 主流程編排
 *
 * 依賴外部 Library:
...
class SprintService {
  ...
  preview() {
    ...
    if (latest) {
      ...
      const { start, end } = this._calcNextSprint(latest.endDate);
      newName = `${this._formatMMDD(start)}-${this._formatMMDD(end)}`;
      Logger.log(`\n📆 下一個 Sprint:${newName}`);
      Logger.log(`   起始日:${this._formatDate(start)}(${this._formatWeekday(start)})`);
      Logger.log(`   結束日:${this._formatDate(end)}(${this._formatWeekday(end)})`);
    } else {
  ...
  listAll() {
    ...
    Logger.log(`📋 共找到 ${sprints.length} 個 Sprint(${this.year} 與 ${this.year - 1} 年度):`);
    sprints.forEach((s, i) => {
      const marker = i === 0 ? '⭐' : '  ';
  ...
  /**
   * 列出今年與去年的 Sprint(依結束日新到舊)
   *
   * 跟 SprintFinder.js 共用同一份邏輯,含去年是為了涵蓋跨年 Sprint
   * (例如歸檔在 2026/ 但要到 2027/01 才結束的 1228-0108)。
   * @private
   */
  _listAllSprints() {
    return listRecentSprintFolders(this.drive, this.sprintRootFolderId);
  }
