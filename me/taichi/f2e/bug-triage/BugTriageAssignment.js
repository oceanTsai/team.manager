...
//   7. getAssignee_() 依象限水平 + domain 從值星表挑人
 ...
 *
 * 資料夾結構：
 *   線上問題/          ← 這個資料夾的 ID 填在 config.js 的 CONFIG_DRIVE_ROOT_FOLDER_ID
 *     2026/
...
function doPost(e) {
  try {
    ...
    const assignee = getAssignee_(ss, aiResult.quadrant.levels, aiResult.domain);
 ...
 *
 * 根資料夾 ID 設定在 config.js 的 CONFIG_DRIVE_ROOT_FOLDER_ID（寫死常數，不是指令碼屬性）。
 *
...
function getSpreadsheet_() {
  ...
  if (!rootId) throw new Error("config.js 的 CONFIG_DRIVE_ROOT_FOLDER_ID 未設定");
 ...
 *
 *   注意：這個函式只負責「挑人」，不會寫回任何試算表；
 *   實際的派工結果由呼叫端（doPost）寫入「線上問題」G 欄。
 *
 ...
 */
function getAssignee_(ss, allowedLevels, domain) {
